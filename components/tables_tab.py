...
class TablesTab(QWidget):
  def __init__(self, db_manager, parent=None):
    ...
    self.tables_table.setHorizontalHeaderLabels(["Номер стола", "Вместимость", "Действия"])
  ...
  def update_tables_table(self):
    ...
    for row_number, table in enumerate(tables):
      ...
      self.tables_table.setItem(row_number, 0, QTableWidgetItem(str(table[0])))
      self.tables_table.setItem(row_number, 1, QTableWidgetItem(str(table[1])))
...
class EditTableDialog(QDialog):
  ...
  def save_changes(self):
    table_number = self.table_number_input.text()
    capacity = self.capacity_input.text()
    # ... (Логика получения table_id)
    # ... (db_manager.update_table(table_id, table_number, capacity))
    self.close()
